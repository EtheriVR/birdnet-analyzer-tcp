...
def file_list_in_directory():
    all_files = []
    st.write("Testing")
    for i in os.listdir("/app/recordings") :
        st.write(i)
        st.audio("/app/recordings/"+i, loop=False)

st.set_page_config(layout="wide") # Use wide layout for better table display

main, data, audio = st.tabs(["Main", "Data", "Audio"])

# --- Configuration ---
API_URL = "http://bird-api_server:8080/birds" # The endpoint to fetch data from
...
with main:
    ...
    try:
        st.audio("/app/recordings/"+get_recent_bird()+".wav", loop=False)
    except:
        pass
            ...
            try:
                ...
                try:
                    ...
                    if isinstance(data, list) and data:
                        ...
                        if 'name' in df.columns and 'created_at' in df.columns and 'confidence' in df.columns:
                            # Display the data in a table
                            st.subheader("Fetched Bird Data")
                            st.dataframe(df[['name','confidence', 'created_at']], use_container_width=True) # Select and display only name and date
                        else:
